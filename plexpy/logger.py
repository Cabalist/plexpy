        ...
        for item in _BLACKLIST_WORDS:
            try:
                ...
                if any(item in str(arg) for arg in record.args):
                    record.args = tuple(arg.replace(item, 8 * '*' + item[-2:]) if isinstance(arg, string_types) else arg
...
class PublicIPFilter(logging.Filter):
    ...
    def filter(self, record):
        ...
        try:
            ...
            args = []
            for arg in record.args:
                ipv4 = re.findall(r'[0-9]+(?:\.[0-9]+){3}(?!\d*-[a-z0-9]{6})', arg) if isinstance(arg, string_types) else []
                for ip in ipv4:
                    if helpers.is_ip_public(ip):
